    ...
    mountRoutes() {
        ...
        router.get('/:code', (req, res) => {
            ...
            if (req.params.code == 'sites') {
                ...
                console.log('PEDIDO ' + req.params.code);
        ...
        router.post('/sites/:site', (req, res) => {
            ...
            if (this.validURL(req.params.site)) {
                this.postSite('sites', req.params.site).then((site) => {
                    res.status(201).send({
                        status: 'success',
                        data: site
                    });
    ...
    getSite(name, query, cb) {
        mongoose.connection.db.collection(name, (err, collection) => {
            ...
            collection.findOneAndUpdate(query, newVal, (e, res) => {
                if (e) {
                    if (err)
                        return res.status(500).send({ status: 'fail', code: '500', message: 'The server encountered an unexpected condition which prevented it from fulfilling the request.', error: err });
                    res.toArray(cb);
                }
